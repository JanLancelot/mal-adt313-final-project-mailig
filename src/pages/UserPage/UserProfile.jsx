...
export default function UserProfile() {
  ...
  const reviewsWithTitles = useMemo(() => {
    return reviews.map((review) => {
      const anime = animeList.find((a) => a.id === review.animeId);
      return {
        ...review,
  ...
  return (
    <div className="user-page-container">
      ...
      <div className="reviews-list">
        {reviewsWithTitles.length === 0 ? (
          <p>No reviews yet.</p>
        ) : (
          reviewsWithTitles.map((review) => (
            <div key={review.animeId} className="review-item">
              <p
                className="review-anime-title"
                onClick={() => handleAnimeClick(review.animeId)}
              >
                {review.title}
              </p>
